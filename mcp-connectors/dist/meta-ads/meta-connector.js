...
export class MetaAdsConnector extends BaseAdConnector {
    ...
    async testConnection() {
        try {
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/me?fields=id,name`);
            return {
                success: true,
                data: true
            };
    ...
    async createCampaign(campaign) {
        try {
            ...
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${this.config.accountId}/campaigns`, metaCampaignData);
            if (!response.success) {
                return response;
            }
    ...
    async updateCampaign(campaignId, updates) {
        try {
            ...
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${campaignId}`, updateData);
            if (!response.success) {
                return response;
            }
    ...
    async getCampaign(campaignId) {
        try {
            ...
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${campaignId}?fields=${fields}`);
            if (!response.success) {
                return response;
            }
    ...
    async listCampaigns(filters) {
        try {
            ...
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${this.config.accountId}/campaigns?${queryString}`);
            if (!response.success) {
                return response;
            }
            const campaigns = response.data.data.map((metaCampaign) => ({
                id: metaCampaign.id,
                name: metaCampaign.name,
                status: this.mapMetaStatusToGAELP(metaCampaign.status),
                objective: { type: this.mapMetaObjectiveToGAELP(metaCampaign.objective) },
    ...
    async deleteCampaign(campaignId) {
        try {
            const response = await this.makeApiRequest('DELETE', `${this.baseUrl}/${campaignId}`);
            return {
    ...
    async updateCampaignStatus(campaignId, status) {
        try {
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${campaignId}`, { status });
            return {
                success: response.success,
                data: response.success
            };
    ...
    async uploadCreative(creative) {
        try {
            ...
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${this.config.accountId}/adcreatives`, creativeData);
            if (!response.success) {
                return response;
            }
    ...
    async uploadImage(imageUrl) {
        try {
            ...
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${this.config.accountId}/adimages`, { url: imageUrl });
            return response;
        }
        catch (error) {
    ...
    async getCreative(creativeId) {
        try {
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${creativeId}?fields=id,name,object_story_spec`);
            if (!response.success) {
                return response;
            }
    ...
    async listCreatives(filters) {
        try {
            ...
            const queryString = buildQueryString(params);
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${this.config.accountId}/adcreatives?${queryString}`);
            if (!response.success) {
                return response;
            }
            const creatives = response.data.data.map((metaCreative) => ({
                id: metaCreative.id,
                name: metaCreative.name,
                type: 'IMAGE',
                assets: {
    ...
    async deleteCreative(creativeId) {
        try {
            const response = await this.makeApiRequest('DELETE', `${this.baseUrl}/${creativeId}`);
            return {
                success: response.success,
                data: response.success
            };
    ...
    async getCampaignPerformance(campaignId, dateRange, metrics) {
        try {
            ...
            const queryString = buildQueryString(params);
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${campaignId}/insights?${queryString}`);
            if (!response.success) {
                return response;
            }
    ...
    extractConversions(actions) {
        ...
        const conversionActions = actions.filter(action => action.action_type === 'purchase' ||
    ...
    async createAudience(audience) {
        try {
            ...
            const response = await this.makeApiRequest('POST', `${this.baseUrl}/${this.config.accountId}/customaudiences`, audienceData);
            if (!response.success) {
                return response;
            }
    ...
    async getAudience(audienceId) {
        try {
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${audienceId}?fields=id,name,description,approximate_count,retention_days`);
            if (!response.success) {
                return response;
            }
    ...
    async listAudiences(filters) {
        try {
            ...
            const queryString = buildQueryString(params);
            const response = await this.makeApiRequest('GET', `${this.baseUrl}/${this.config.accountId}/customaudiences?${queryString}`);
            if (!response.success) {
                return response;
            }
            const audiences = response.data.data.map((metaAudience) => ({
                id: metaAudience.id,
                name: metaAudience.name,
                type: 'CUSTOM',
                description: metaAudience.description,
    ...
    async deleteAudience(audienceId) {
        try {
            const response = await this.makeApiRequest('DELETE', `${this.baseUrl}/${audienceId}`);
            return {
                success: response.success,
                data: response.success
            };
    ...
    mapMetaObjectiveToGAELP(objective) {
        const mapping = {
            'BRAND_AWARENESS': 'BRAND_AWARENESS',
            'REACH': 'REACH',
            'LINK_CLICKS': 'TRAFFIC',
    ...
    mapMetaBidStrategyToGAELP(bidStrategy) {
        const mapping = {
