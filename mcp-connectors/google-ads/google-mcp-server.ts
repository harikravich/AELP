...
export class GoogleAdsMCPServer {
  ...
  private connector: GoogleAdsConnector | null = null;
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'google_connect':
            return await this.handleConnect(args as unknown as GoogleAdsMCPConfig);
  ...
  private async handleConnect(config: GoogleAdsMCPConfig) {
    try {
  ...
  private async handleUpdateCreative(args: any) {
    await this.ensureConnected();
