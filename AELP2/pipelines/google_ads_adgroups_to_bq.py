#!/usr/bin/env python3
"""
Load Google Ads ad group performance into BigQuery (ads_ad_group_performance).
Redacts campaign/ad_group names by default.
"""
...
def run(start: str, end: str, customer_id: str):
    ...
    ensure_and_insert(project, dataset, "ads_ad_group_performance", schema(), out_rows, partition_field="date")
    logger.info(f"Inserted {len(out_rows)} rows into {dataset}.ads_ad_group_performance for customer {customer_id}")
