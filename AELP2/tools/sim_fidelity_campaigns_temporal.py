#!/usr/bin/env python3
"""
Phase 2 fidelity: temporal patterns (weekday seasonality + frequency fatigue)

Builds on Phase 1 per-campaign heterogeneity, and adds:
- Weekday multipliers for CVR per campaign with shrinkage to account-level
- Frequency fatigue: log(CVR) ~ k*(freq - median_freq_train), clamp effect

Outputs
- JSON: AELP2/reports/sim_fidelity_campaigns_temporal.json
"""
...
def _beta_hyper_from_global(success: float, total: float, k: float = 120.0):
...
def _simulate_day(spend: float, mu: float, sigma: float, cpc_lo: float, cpc_hi: float,
                  ...
                  draws: int = 200) -> Tuple[float, float, float]:
...
def main():
    ...
    # Weekday multipliers and frequency slopes per campaign
    wd_mults: Dict[str, Dict[int, float]] = {}
    freq_slope: Dict[str, float] = {}
    med_freq: Dict[str, float] = {}
    params: Dict[str, dict] = {}

    # account-level weekday ratios for shrinkage
    ...
    for cid, rows in by_camp.items():
        ...
        # Weekday multipliers (campaign-level, shrink to account-level)
        ...
        if len(cvrs_pos) >= 1:
            eps = float(np.percentile(np.array(cvrs_pos), 5))
        else:
            eps = 1e-5
        eps = max(1e-6, eps)
        for r in rows:
            if r["date"] not in train_days or r["clicks"] <= 0:
                continue
            cvr = r["purch"] / r["clicks"] if r["clicks"] > 0 else 0.0
        ...
        med_freq[cid] = m_f
        params[cid] = {"mu": mu, "sigma": sigma, "cpc_p5": p5, "cpc_p95": p95, "alpha": a0 + purch_tot, "beta": b0 + max(0.0, clicks_tot - purch_tot), "name": rows[0]["name"] if rows else cid}
    ...
    for cid, rows in by_camp.items():
        ...
        for r in rows:
            ...
            # Frequency effect for the day (centered at train median)
            f = r["freq"]
            slope = freq_slope[cid]
            m_f = med_freq[cid]
            freq_eff = math.exp(slope * (f - m_f))
            freq_eff = float(np.clip(freq_eff, 0.7, 1.2))
            med, p10, p90 = _simulate_day(spend, p["mu"], p["sigma"], p["cpc_p5"], p["cpc_p95"], p["alpha"], p["beta"], wd_mult, freq_eff, draws=200)
            day_pred[d]["med"] += med
            day_pred[d]["p10"] += p10
            day_pred[d]["p90"] += p90
    ...
    path = "AELP2/reports/sim_fidelity_campaigns_temporal.json"
...
if __name__ == "__main__":
    np.random.seed(int(os.getenv("AELP2_SEED", "43")))
    main()
