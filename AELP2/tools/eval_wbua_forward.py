...
"""
Forward-holdout WBUA: freeze decisions using only history up to week t-1 and
evaluate on week t. Produces accuracy and bootstrap 95% CI across campaign-weeks.

Outputs: AELP2/reports/wbua_forward.json
"""
...
OUT = ROOT/ 'AELP2' / 'reports' / 'wbua_forward.json'
...
def main():
    ...
    for cid, flist in by_c.items():
        ...
        for i in range(1, len(weeks)):
            # Baseline is chosen only from t-1 window (or t-4..t-1 if available)
            cur=weeks[i]
    ...
    OUT.write_text(json.dumps({'wbua_forward': wbua, 'pairs': len(pairs), 'ci95_groups': [lo, hi], 'groups': len(group_acc)}, indent=2))
    print(json.dumps({'wbua_forward': wbua, 'pairs': len(pairs), 'groups': len(group_acc), 'out': str(OUT)}, indent=2))
