...
export async function POST() {
  try {
    ...
    // Log a backfill request in ops_flow_runs (acts as a job queue/audit trail)
    ...
    await bq.dataset(dataset).table('ops_flow_runs').insert([{ id, flow:'backfill_policy_enforcement', status:'queued', created_at: new Date(), meta: { source:'external_ui' } }])
